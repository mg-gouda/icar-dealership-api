...
export class UsersService {
  ...
  async findById(id: string) {
    const user = await this.prisma.user.findUnique({
      ...
      select: {
        id: true, email: true, name: true, phone: true, role: true,
        locationId: true, isActive: true, createdAt: true, updatedAt: true,
        location: { select: { id: true, name: true } },
        permissions: { select: { id: true, permissionKey: true, granted: true } },
        workingHours: true,
      },
    });
  ...
  async grantPermission(userId: string, permissionKey: string, granted: boolean, grantedByUserId: string) {
  ...
  async revokePermission(userId: string, permissionKey: string, revokedByUserId: string) {
    ...
    await this.prisma.userPermission.delete({ where: { userId_permissionKey: { userId, permissionKey } } });
    this.audit.log({
      entity: 'UserPermission',
  ...
  async upsertWorkingHours(userId: string, hours: Array<{ dayOfWeek: number; startTime: string; endTime: string }>) {
    await this.findById(userId);
    await this.prisma.$transaction(
      hours.map((h) =>
        this.prisma.workingHours.upsert({
          ...
          create: { userId, dayOfWeek: h.dayOfWeek, startTime: h.startTime, endTime: h.endTime },
        }),
      ),
    );
