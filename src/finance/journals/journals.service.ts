...
export class JournalsService {
  ...
  async create(data: {
    ...
    if (existing) throw new BadRequestException(`Journal code ${data.code} already exists`);

    const journal = await this.prisma.journal.create({ data: data as any });
    await this.audit.log({
      userId, action: 'CREATE', entity: 'Journal', entityId: journal.id,
    });
    return journal;
  }
  ...
  async update(id: string, companyId: string, data: {
    ...
    const journal = await this.prisma.journal.findFirst({ where: { id, companyId } });
    if (!journal) throw new NotFoundException('Journal not found');

    const updated = await this.prisma.journal.update({ where: { id }, data: data as any });
    await this.audit.log({
      userId, action: 'UPDATE', entity: 'Journal', entityId: id,
  ...
  async delete(id: string, companyId: string, userId: string) {
    ...
    const entryCount = await this.prisma.journalEntry.count({ where: { journalId: id } });
    if (entryCount > 0) {
    ...
    await this.audit.log({
      userId, action: 'DELETE', entity: 'Journal', entityId: id,
    });
    return { deleted: true };
  }
}
