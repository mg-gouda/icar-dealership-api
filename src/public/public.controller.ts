  ...
  async dealStatus(@Query('email') email: string, @Query('ref') ref?: string) {
    ...
    const deals = await this.prisma.deal.findMany({
      ...
      select: {
        ...
        installmentPlan: {
          select: {
            id: true, principalAmount: true, downPayment: true,
            durationMonths: true, interestRate: true, startDate: true, status: true,
            installments: {
              select: { dueDate: true, status: true, totalDue: true, paidAmount: true },
              orderBy: { dueDate: 'asc' },
              take: 3,
            },
          },
